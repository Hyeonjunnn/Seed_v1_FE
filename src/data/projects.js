export const projects = [
    ...
    details: '이 시스템은 로그인과 권한 관리 기능을 통해 사용자 인증과 역할 기반 접근 제어를 제공합니다. 고객 데이터는 엑셀 또는 CSV 파일로 쉽게 업로드할 수 있으며, 구독 현황 분석을 통해 전체 구독자, 활성 상태, 이탈 현황을 직관적으로 확인할 수 있습니다. 또한 세그먼트 분석으로 장르 선호, 최근 접속일, 구독 상태 등 다양한 조건별 고객 분류가 가능하고, 코호트 분석을 통해 가입 시점별 장기 구독 유지율을 분석합니다. 더불어 머신러닝 기반 이탈 예측 기능으로 이탈 가능성이 높은 사용자를 식별하며, 구독 상품의 결제와 갱신 기능도 제공합니다.',
    features: [
        'OTT 구독 데이터 수집 및 분석',
        'OpenAI 기반 사용자 이탈 예측 모델 적용',
        '코호트 분석 및 시각화 대시보드 구현',
        'Spring Boot + Flask 연동 구조 설계',
    ...
    details: 'K-PaaS 개발 공모전을 수상을 목표로 진행중입니다. AI 기반 국내 여행 일정 추천과 LLM 활용 일정 생성 기능 서비스를 목표로 개발 중입니다. 사용자 입력 기반으로 AI가 추천하는 여행 일정을 제공하며, NHN K-PaaS 클라우드 환경에 배포되어 있습니다. 현재는 프론트엔드 40% 완료, 백엔드 API 개발 70% 완료, GitHub Actions를 이용한 CI/CD 파이프라인 구축 설계 진행중입니다. 향후 AI 모델 개선 및 사용자 피드백 반영을 통해 지속적으로 발전시킬 예정입니다.',
    features: [
        '사용자 입력 기반 AI 일정 추천',
        '여행 일정 저장 및 공유 기능',
        'Spring Boot + Flask API 연동',
